const raw = `plaid beige bags contain 3 drab magenta bags.
...
const input = raw.split("\n").map(rule => {
    const [outside, inside] = rule.split(/contain /)
    const outsideWords = outside.split(" ")
    const insideRules = inside.split(", ")
    const insideRulesConcise = insideRules.map(rule => {
        const words = rule.split(" ");
        return [words[0], [words[1], words[2]].join(" ")]
    })
    return [[outsideWords[0],outsideWords[1]].join(" "), insideRulesConcise]
})

...
const passportsObjs = passports.reduce((acc,el) => {
    const obj = {};
    el.forEach(property => {
        const key = property.substr(0,3)
        const value = property.substr(4,property.length)
        obj[key] = value
    })
    acc.push(obj)
    return acc
